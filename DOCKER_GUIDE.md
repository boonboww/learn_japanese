# 🐳 Hướng Dẫn Sử Dụng Docker & PostgreSQL Cho Learn Japanese

Chào mừng bạn đến với tài liệu hướng dẫn vận hành backend **learn_japanese** được container hóa hoàn toàn bằng **Docker** và sử dụng cơ sở dữ liệu **PostgreSQL** mạnh mẽ! 
...
Để đảm bảo hai dự án chạy song song mượt mà, **learn_japanese** được cấu hình sử dụng các cổng hoàn toàn độc lập sau:
...
Mở Terminal tại thư mục gốc `learn_japanese` và chạy:
```bash
docker compose up -d
```
> [!TIP]
